      ...
      if (!err) {
        for (let key in values) {
          ...
          if (moment.isMoment(values[key])) {
            //格式化一下时间 YYYY-MM类型
            if (
              moment(values[key].format("YYYY-MM"), "YYYY-MM", true).isValid()
            ) {
        ...
        <Spin spinning={spinning}>
                  ...
                  <Col style={{ width: "100%", textAlign: "right" }}>
                    ...
                    <Button
                      ...
                      onClick = {() => {
                        ...
                        searchOption.onResetFields &&
                          searchOption.onResetFields();

                        //手动触发一下是因为使用resetFields()不会触发form的onValuesChange
                        searchOption.getFieldsValues &&
                          searchOption.getFieldsValues({});
                        searchOption.onFieldsChange &&
                          searchOption.onFieldsChange({});
                      }}
                    >
                      重置
          ...
          >
            <Row gutter={24}>
              <Col
                span={
                  (cardTableOption.rowCol && cardTableOption.rowCol[0]) || 6
                }
              >
                <Card
              ...
              <Col
                span={
                  (cardTableOption.rowCol && cardTableOption.rowCol[1]) || 18
                }
              >
                <Card
                  ...
                  bodyStyle={{
                                                                                                                paddingBottom              : 
                                                                                         tableOption.pagination || tableOption.pageSize ? 0: 8
                  }}
                >
                  <AsyncTable
                    url        = {tableOption.url}
...
export default Form.create({
  onValuesChange: (props, values) => {
    //给外部一个获得搜索条件的回调
    props.searchOption.getFieldsValues &&
      props.searchOption.getFieldsValues(values);
    props.searchOption.onFieldsChange &&
      props.searchOption.onFieldsChange(values);
  }
})(TreeTable);
